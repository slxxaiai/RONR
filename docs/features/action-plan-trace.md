...
定义最终 Action Plan 如何追溯到 Speech、Objection、Vote、Reservation 和用户插话。
...
3. 每个 Action Item 引用相关 Speech、Objection 和 Vote。
...
- 自动化测试：覆盖完整 trace、缺少 Speech、缺少 Vote、Reservation 被误放入 Vote.position。
- fixture 验证：准备一组完整议事记录和一组缺失证据链记录。
